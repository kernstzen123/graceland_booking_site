...
create unique index if not exists payfast_provider_reference_unique
  on public.payments (provider_reference)
  where method = 'PAYFAST' and provider_reference is not null;
create unique index if not exists one_payfast_payment_per_booking
  on public.payments (booking_id)
  where method = 'PAYFAST';

drop function if exists public.reserve_capacity(date, integer, jsonb, text, numeric, text);
...
begin
  ...
  ) then raise exception 'That birthday party time slot has already been booked.'; end if;

  select coalesce(sum(people_count), 0) into v_current_count from public.bookings
  where visit_date = p_visit_date and status in ('CONFIRMED', 'PAID', 'PAYMENT_PENDING', 'UNPAID')
    and (status != 'UNPAID' or expires_at > now());
  ...
  values (p_customer->>'firstName', p_customer->>'lastName', p_customer->>'email', p_customer->>'phone') returning id into v_customer_id;
  insert into public.bookings (reference, customer_id, visit_date, status, total_amount, people_count, expires_at, party_slot, idempotency_key)
  values (p_reference, v_customer_id, p_visit_date, 'UNPAID', p_total_amount, p_people_count, now() + interval '15 minutes', p_party_slot, p_idempotency_key)
