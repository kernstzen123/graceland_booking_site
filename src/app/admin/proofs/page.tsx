...
export default function ReviewProofs() {
  ...
  const act = async (proof: Proof, action: 'approve' | 'reject', force = false) => {
    ...
    if (!session) return;
    const res = await fetch('/api/admin/proofs', { method: 'POST', headers: { Authorization: `Bearer ${session.access_token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ proofId: proof.id, action, reason, force }) });
    ...
    if (data.emailSent === false && data.message) {
      setActionMessage({ proofId: proof.id, text: data.message, type: 'warning' });
    }
    await load();
  };
